...
local lang,isOpen,korblox,headless,espEnabled,noclipEnabled,flyEnabled,infJumpEnabled,currentTab,skyIdx,animIdx,menuMusicPlaying,speedEnabled,jumpBoostEnabled,cloneSkinEnabled,bigHeadEnabled,allBigHeadEnabled,scarySkinsEnabled = "RU",false,false,false,false,false,false,false,"Visual",1,1,true,false,false,false,false,false,false
...
pcall(function() soundObj.Parent = P:WaitForChild("PlayerGui") end)
...
local T={
	RU={
		Title="УНИВЕРСАЛЬНОЕ МЕНЮ",Auth="TikTok:ScriptVFXK",TabVisual="Визуал",TabPlayer="Игрок",TabMods="Моды",
		Sky="Небо: ",MenuMusic="Музыка меню: ",Anim="Пак анимаций: ",InfJump="Бесконечный прыжок: ",NC="Ноклип: ",
		FLY="Полёт: ",Speed="Быстрый бег: ",JumpBoost="Супер прыжок: ",TP="Телепорт к игроку",TPPlaceholder="Введите ник игрока...",
		CloneSkin="Клонировать скин (Визуал)",BigHead="Большая голова (Визуал)",AllBigHead="Головы всем (Визуал)",
		ScarySkins="Страшные скины всем",Korblox="Фейк Корблокс: ",Headless="Фейк Хедлесс: ",ESP="ЕСП: ",
		ON="Вкл",OFF="Выкл",UserTag="Скрипт юзер / Script User",
		LoadingSec="Загрузка (%d) сек...",LoadingDone="Загрузка завершена!",
		LangSelect="ScriptVFXK приветствует!\nВыберите язык / Select language:"
	},
	EN={
		Title="UNIVERSAL MENU",Auth="TikTok:ScriptVFXK",TabVisual="Visual",TabPlayer="Player",TabMods="Mods",
		Sky="Sky: ",MenuMusic="Menu Music: ",Anim="Anim Pack: ",InfJump="Infinite Jump: ",NC="Noclip: ",
		FLY="Fly: ",Speed="Fast Speed: ",JumpBoost="Super Jump: ",TP="Teleport to Player",TPPlaceholder="Enter player username...",
		CloneSkin="Clone Skin (Visual)",BigHead="Big Head (Visual)",AllBigHead="All Big Head (Visual)",
		ScarySkins="Scary Skins for All",Korblox="Fake Korblox: ",Headless="Fake Headless: ",ESP="ESP: ",
		ON="ON",OFF="OFF",UserTag="Скрипт юзер / Script User",
		LoadingSec="Loading (%d) sec...",LoadingDone="Loading Complete!",
		LangSelect="ScriptVFXK welcomes you!\nSelect language / Выберите язык:"
	}
}
...
-- СИСТЕМА МАРКИРОВКИ Script User
local userTags = {}
local function createHeadTag(plr)
	if not plr or not plr.Character or plr == P then return end
	...
	local bg = cObj("BillboardGui", head, {Name="GooseScriptUserTag", Adornee=head, Size=UDim2.new(0,200,0,30), StudsOffset=Vector3.new(0,2.5,0), AlwaysOnTop=true})
...
-- СТРАШНЫЕ СКИНЫ ДЛЯ ВСЕХ ИГРОКОВ
local function applyScarySkin(targetChar, targetPlayer)
	if not scarySkinsEnabled or not targetChar then return end
	savePlayerOriginalLook(targetChar, targetPlayer)
	for _, child in ipairs(targetChar:GetChildren()) do
		if child:IsA("Accessory") or child:IsA("Clothing") then
			child:Destroy()
		elseif child:IsA("BasePart") then
			child.Color = Color3.fromRGB(15, 15, 15)
			child.Material = Enum.Material.SmoothPlastic
		end
	end
	local head = targetChar:FindFirstChild("Head")
	if head then
		for _, d in ipairs(head:GetChildren()) do if d:IsA("Decal") then d:Destroy() end end
		local scaryFace = cObj("Decal", head, {Name="ScaryFace", Texture="rbxassetid://154205518", Face=Enum.NormalId.Front})
	end
end

local function updateScarySkins()
	for _, plr in ipairs(Pl:GetPlayers()) do
		if plr.Character then
			if scarySkinsEnabled then
				applyScarySkin(plr.Character, plr)
			else
...
Pl.PlayerAdded:Connect(function(plr)
	plr.CharacterAdded:Connect(function(char)
		task.wait(1)
		if scarySkinsEnabled then applyScarySkin(char, plr) end
...
local TabBar = cObj("ScrollingFrame", Main, {Size=UDim2.new(1,-14,0,32), Position=UDim2.new(0,7,0,38), BackgroundTransparency=1, CanvasSize=UDim2.new(0,300,0,0), ScrollBarThickness=0, Active=false, ZIndex=10})
...
local ScrollPlayer = cObj("ScrollingFrame", Main, {Size=UDim2.new(1,0,1,-90), Position=UDim2.new(0,0,0,72), BackgroundTransparency=1, CanvasSize=UDim2.new(0,0,0,460), ScrollBarThickness=4, ScrollBarImageColor3=Color3.fromRGB(255,200,100), Active=true, Visible=false, ZIndex=3})
local ScrollMods = cObj("ScrollingFrame", Main, {Size=UDim2.new(1,0,1,-90), Position=UDim2.new(0,0,0,72), BackgroundTransparency=1, CanvasSize=UDim2.new(0,0,0,250), ScrollBarThickness=4, ScrollBarImageColor3=Color3.fromRGB(255,200,100), Active=true, Visible=false, ZIndex=3})
...
local ScarySkinsB = btn(ScrollMods,210,Color3.fromRGB(35,35,50)) ScarySkinsB.Size,ScarySkinsB.Position = UDim2.new(0.86,0,0,36),UDim2.new(0.07,0,0,210)
...
local function updateUI()
	...
	ScarySkinsB.Text = "👹 "..d.ScarySkins..": "..(scarySkinsEnabled and d.ON or d.OFF)
	ScarySkinsB.BackgroundColor3 = scarySkinsEnabled and Color3.fromRGB(40,130,75) or Color3.fromRGB(35,35,50)
...
ScarySkinsB.MouseButton1Click:Connect(function() scarySkinsEnabled = not scarySkinsEnabled updateScarySkins() updateUI() end)
...
P.CharacterAdded:Connect(function(c)
	...
	if scarySkinsEnabled then task.wait(1) updateScarySkins() end
